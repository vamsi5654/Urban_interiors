...
import { Camera, Upload, Edit3, Trash2, Plus, FolderPlus, Eye, X, Check, AlertTriangle } from 'lucide-react';
...
const mockCustomers = [
  {
    ...
    rooms: {
      bedroom: [
        { id: 'img1', filename: 'bed1.jpg', path: '/uploads/cust123/bedroom/bed1.jpg', uploadedAt: '2025-07-21' },
        { id: 'img2', filename: 'bed2.jpg', path: '/uploads/cust123/bedroom/bed2.jpg', uploadedAt: '2025-07-21' }
      ],
      kitchen: [
        { id: 'img3', filename: 'kitchen1.jpg', path: '/uploads/cust123/kitchen/kitchen1.jpg', uploadedAt: '2025-07-21' }
      ],
      poojaRoom: [],
      livingRoom: [],
      bathroom: [],
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const newCustomer = {
        ...
        rooms: {} // to hold room images
      };

      const cloudName = 'omsi';        // Your Cloudinary cloud name
      const uploadPreset = 'urbann_preset';  // Your upload preset
...
const RoomEditorModal = ({ isOpen, onClose, mode, roomKey, roomLabel, onSave, onUpload }) => {
  ...
  const handleSave = () => {
    if (mode === 'edit' && newRoomName.trim()) {
      onSave(roomKey, newRoomName.trim());
    } else if (mode === 'add' && newRoomType) {
      onSave(newRoomType, newRoomType);
    } else if (mode === 'upload' && selectedFiles.length > 0) {
      onUpload(roomKey, selectedFiles);
    }
    handleClose();
  };
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title={
      ...
      <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end' }}>
        ...
        >
          <Check size={16} />
          {mode === 'edit' ? 'Save Changes' : mode === 'add' ? 'Add Room' : 'Upload Images'}
        </Button>
      </div>
    </Modal>
  );
...
const OutletFolder = ({ customer, onBack, onViewRoom, onUpdateCustomer }) => {
  const [editorModal, setEditorModal] = useState({ isOpen: false, mode: '', roomKey: '', roomLabel: '' });
  const [confirmDialog, setConfirmDialog] = useState({ isOpen: false, roomKey: '', roomLabel: '' });
  ...
  const handleDeleteRoom = (roomKey, roomLabel) => {
    setConfirmDialog({ isOpen: true, roomKey, roomLabel });
  };
  ...
  const confirmDeleteRoom = () => {
    const updatedCustomer = {
      ...
      rooms: {
        ...customer.rooms,
        [confirmDialog.roomKey]: []
      }
    };
    onUpdateCustomer(updatedCustomer);
    setConfirmDialog({ isOpen: false, roomKey: '', roomLabel: '' });
  };

  const handleSaveRoom = (roomKey, newName) => {
    if (editorModal.mode === 'add') {
      ...
      onUpdateCustomer(updatedCustomer);
    } else if (editorModal.mode === 'edit') {
      // For simplicity, we'll just update the display name
      // In a real app, you'd handle room key changes properly
      onUpdateCustomer(customer);
    }
    setEditorModal({ isOpen: false, mode: '', roomKey: '', roomLabel: '' });
  };

  const handleUploadImages = (roomKey, files) => {
    const newImages = files.map((file, index) => ({
      id: 'img' + Date.now() + index,
      filename: file.name,
      path: `/uploads/${customer.id}/${roomKey}/${file.name}`,
  ...
  return (
    <div>
      ...
      <ConfirmDialog
        ...
        onClose={() => setConfirmDialog({ isOpen: false, roomKey: '', roomLabel: '' })}
        onConfirm={confirmDeleteRoom}
        title="Delete Room Folder"
        message={`Are you sure you want to delete the "${confirmDialog.roomLabel}" folder? This will permanently remove all ${(customer.rooms && customer.rooms[confirmDialog.roomKey]) ? customer.rooms[confirmDialog.roomKey].length : 0} images in this room.`}
        type="danger"
      />
    </div>
  );
...
const CategoryGallery = ({ category, allCustomers, onBack }) => {
  ...
  // Collect all images from this category across all customers
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto' }}>
        ...
        {currentView === 'room-gallery' && selectedCustomer && selectedRoom && (
          <RoomGallery
            ...
            onAddImages={(roomKey) => {
              // Update the selected customer to get fresh data
