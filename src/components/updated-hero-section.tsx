...
import AdminChatBoard from './AdminChatBoard';
...
const HeroSection = () => {
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Set current customer ID and show chat
      setCurrentCustomerId(docRef.id);
      ...
      await addDoc(messagesRef, {
        text: `Hello ${formData.fullName}! Thank you for your interest in Urbann Interiors. How can we help you today?`,
        sender: 'admin',
        senderName: 'Support Team',
        timestamp: serverTimestamp()
      });

      console.log('Customer added and chat initiated');
    } catch (error) {
  ...
  // Admin access (for demo purposes - you'd want proper authentication)
  ...
  // If admin board is open, show admin interface
  if (showAdminBoard) {
    return (
      <AdminChatBoard onClose={() => setShowAdminBoard(false)} />
    );
  }

  return (
    <>
      {/* Admin Access Button (hidden in top corner) */}
      <button
      ...
      <section style={{
        ...
        <div style={{
          ...
          {isMediumAndAbove && !isFormSubmitted && (
            <div style={{
              flex: '0 0 auto',
              display: 'flex',
              alignItems: 'center',
        ...
        {!isMediumAndAbove && !isFormSubmitted && (
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {/* Mobile form fields - same as desktop */}
                <div>
                  <input
                    type="text"
                    name="fullName"
